      ...
      <div className="space-y-1 mt-2">
        {techCategories.map((category, index) => (
          <div key={index} className="space-y-1">
            <h3 className="font-manuscript text-sm font-semibold dark:text-title">
              {category.name}
            </h3>
            <div className="relative overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-amber-800/30 scrollbar-track-transparent">
              <div className="flex space-x-3 pb-1 lg:flex-wrap">
                {category.techs.map((tech, techIndex) => (
                  <motion.div
                    ...
                    className="px-3 py-1 lg:mt-2 text-xs whitespace-nowrap rounded-md border border-amber-100/30 dark:border-amber-100/20 dark:text-neutral-500 hover:dark:bg-neutral-800/80 transition-colors"
                  >
                    {tech}
                  </motion.div>
                ))}
